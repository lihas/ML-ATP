...
SyntaxType Syntax,SIGNATURE Signature) {
    ...
    if (Syntax == tptp_fof) {
        sprintf(Axiom,"fof(%s_substitution_%d,axiom,\n    (! [%s] : ( (X = Y) => (",Symbol,Position,Variables);
        strcat(Axiom,TermX);
        strcat(Axiom," = ");
        strcat(Axiom,TermY);
        strcat(Axiom,")))).");
    ...
    if (Syntax == tptp_cnf) {
        sprintf(Axiom,"cnf(%s_substitution_%d,axiom,\n    ((X != Y) | (%s = %s))).",Symbol,Position,TermX,TermY);
    }
...
SyntaxType Syntax,SIGNATURE Signature) {
    ...
    MakeTermSubstitutionPair(Symbol,Arity,Position,TermX,TermY,Variables);
    if (Syntax == tptp_fof) {
        sprintf(Axiom,"fof(%s_substitution_%d,axiom,\n    (![%s] : ( ((X = Y) & %s ) => %s))).",Symbol,Position,Variables,TermX,TermY);
    }
    if (Syntax == tptp_cnf) {
        sprintf(Axiom,"cnf(%s_substitution_%d,axiom,\n    ((X != Y) | ~%s | %s)).",Symbol,Position,TermX,TermY);
    }
...
//----Add a leading blank line if being pretty
    ...
    if (strchr(WhatEquality,'f') != NULL) {
        ...
        while (Symbol != NULL) {
            Slash = strtok_r(Symbol,"/",&NextSlash);
            Slash = strtok_r(NULL,"/",&NextSlash);
            Arity = atoi(Slash);
            for (Index=1;Index <= Arity;Index++) {
