...
FlatTypeFormula(QuantifiedFormula.VariableType)) {
...
TSTPSyntaxFlag);
//----Nasty nack for $let_ft/$let_tt formulae, which need variables combined 
//----but nothing else pretty. That case sends -1 for Pretty.
                if (Pretty < 0) {
                    PrettyArgument = 0;
                    Pretty = 1;
                } else {
                    PrettyArgument = Pretty;
                }
                while (Pretty && 
...
FormulaUnion.QuantifiedFormula,Indent+4,Pretty,TSTPSyntaxFlag);
                ...
                PFprintf(Stream,"] :");
//----Other half of the $let_ft/$let_tt hack
                Pretty = PrettyArgument;
...
int Pretty) {
    ...
    PFprintf(Stream,"(%s,%s",AnnotatedTSTPFormula.Name,
