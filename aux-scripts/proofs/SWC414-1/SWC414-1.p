...
- <A HREF="http://www.cs.miami.edu/~tptp/cgi-bin/SystemOnTPTP?TPTPProblem=SWC414-1" TARGET="_blank">Solve Problem</A>
</CENTER><HR>
<pre>
%--------------------------------------------------------------------------
...
% Rating   : 0.73 v6.2.0, 0.80 v6.1.0, 0.86 v6.0.0, 0.90 v5.5.0, 0.95 v5.3.0, 1.00 v5.2.0, 0.94 v5.0.0, 0.93 v4.1.0, 0.92 v4.0.1, 1.00 v2.4.0
% Syntax   : Number of clauses     :  241 (  69 non-Horn;  60 unit; 198 RR)
%            Number of atoms       :  770 ( 145 equality)
%            Maximal clause size   :   11 (   3 average)
%            Number of predicates  :   20 (   0 propositional; 1-2 arity)
%            Number of functors    :   62 (  16 constant; 0-2 arity)
%            Number of variables   :  381 (  49 singleton)
%            Maximal term depth    :    6 (   1 average)
% SPC      : CNF_UNS_RFO_SEQ_NHN

% Comments : Created by CNF conversion from SWC414+1
%--------------------------------------------------------------------------
...
<A NAME="co1_7"></A>cnf(co1_7,negated_conjecture,
    ...
    | ~ ssList(D)
    | ~ ssList(E)
    | app(app(app(app(C,cons(A,nil)),D),cons(B,nil)),E) != sk2
    | app(app(app(app(C,cons(B,nil)),D),cons(A,nil)),E) != sk1
    | ~ ssItem(F)
    | ~ ssItem(G)
    | ~ ssList(H)
    | app(app(cons(F,nil),cons(G,nil)),H) != sk4 )).
