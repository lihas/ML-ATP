...
function expand_file(name,            pipe, tmp)
{
   ...
   if(!tmp)
   {
      print "sequential_eprover_raw.awk: Cannot expand filename" > "/dev/stderr";
      exit 1;
   }
   return tmp;
}
  ...
  if(first_job)
    {
      if(!executable)
	{
	  print "sequential_eprover_raw.awk: No executable found" > "/dev/stderr";
	  exit 1;
	}
      if(!logfile)
	{
	  print "sequential_eprover_raw.awk: No logfile found" > "/dev/stderr";
	  exit 1;
	}
      if(!problemdir)
	{
	  print "sequential_eprover_raw.awk: No problemdir found" > "/dev/stderr";
	  exit 1;
	}
